...
TEST_CASE("ActionPromptModal: Color string to design token mapping",
          "[action_prompt][modal][color]") {
    // These tests verify that action:prompt color names map correctly to
    // the project's design tokens. The modal should use ui_theme_get_color()
    // with these token names, not hardcoded hex values.

    SECTION("primary maps to primary_color token") {
        // Expected: "primary" -> ui_theme_get_color("primary_color")
        // The implementation should NOT hardcode colors - it should use
        // design tokens that respect light/dark theme switching.
        std::string color = "primary";
        // When ActionPromptModal is implemented:
        // auto token = ActionPromptModal::map_color_to_token(color);
        // REQUIRE(token == "primary_color");

        // For now, just document the expected behavior
        REQUIRE(color == "primary");
    }

    SECTION("secondary maps to secondary_color token") {
        std::string color = "secondary";
        REQUIRE(color == "secondary");
    }

    SECTION("info maps to info_color token") {
        std::string color = "info";
        REQUIRE(color == "info");
    }

    SECTION("warning maps to warning_color token") {
        std::string color = "warning";
        REQUIRE(color == "warning");
    }

    SECTION("error maps to error_color token") {
        std::string color = "error";
        REQUIRE(color == "error");
    }

    SECTION("empty color defaults to primary") {
        std::string color = "";
        // When implemented: map_color_to_token("") should return "primary_color"
        REQUIRE(color.empty());
    }

    SECTION("unknown color defaults to primary") {
        std::string color = "invalid_color_name";
        // When implemented: map_color_to_token("invalid") should return "primary_color"
        REQUIRE(color == "invalid_color_name");
    }
}
...
TEST_CASE("ActionPromptModal: Construction from PromptData",
          "[action_prompt][modal][construction]") {
    SECTION("Modal can be created with minimal PromptData") {
        PromptData data;
        data.title = "Test Prompt";

        // When ActionPromptModal is implemented:
        // ActionPromptModal modal;
        // REQUIRE(modal.get_name() != nullptr);
        // REQUIRE_THAT(modal.get_name(), Catch::Matchers::Equals("ActionPrompt"));

        REQUIRE(data.title == "Test Prompt");
    }

    SECTION("Modal preserves title from PromptData") {
        PromptData data;
        data.title = "Filament Change Required";

        // When implemented:
        // ActionPromptModal modal;
        // modal.set_prompt_data(data);
        // REQUIRE(modal.get_title() == "Filament Change Required");

        REQUIRE(data.title == "Filament Change Required");
    }

    SECTION("Modal preserves multiple text lines") {
        PromptData data;
        data.title = "Multi-line Test";
        data.text_lines.push_back("Line 1: Please insert filament");
        data.text_lines.push_back("Line 2: Press continue when ready");
        data.text_lines.push_back("Line 3: Or press cancel to abort");

        // When implemented:
        // ActionPromptModal modal;
        // modal.set_prompt_data(data);
        // REQUIRE(modal.get_text_line_count() == 3);
        // REQUIRE(modal.get_text_line(0) == "Line 1: Please insert filament");

        REQUIRE(data.text_lines.size() == 3);
    }

    SECTION("Modal with empty title is valid") {
        PromptData data;
        data.title = "";
        data.text_lines.push_back("Message without title");

        // Empty title is valid per Klipper protocol
        REQUIRE(data.title.empty());
...
TEST_CASE("ActionPromptModal: Button creation from PromptButton list",
          "[action_prompt][modal][buttons]") {
    SECTION("Single button is created correctly") {
        PromptData data;
        data.title = "Confirm";
        ...
        // ActionPromptModal modal;
        // modal.set_prompt_data(data);
        // REQUIRE(modal.get_button_count() == 1);
        // auto btn_info = modal.get_button_info(0);
        // REQUIRE(btn_info.label == "OK");

        REQUIRE(data.buttons.size() == 1);
        REQUIRE(data.buttons[0].label == "OK");
    }

    SECTION("Multiple buttons maintain order") {
        PromptData data;
        data.title = "Choose Action";
        data.buttons.push_back({"Continue", "RESUME", "primary", false, -1});
        data.buttons.push_back({"Pause", "PAUSE", "secondary", false, -1});
        data.buttons.push_back({"Cancel", "ABORT", "error", false, -1});

        // Buttons should appear in the order they were added
        REQUIRE(data.buttons.size() == 3);
        REQUIRE(data.buttons[0].label == "Continue");
        REQUIRE(data.buttons[1].label == "Pause");
        REQUIRE(data.buttons[2].label == "Cancel");
    }

    SECTION("Button labels are displayed correctly") {
        PromptData data;
        data.title = "Test";
        data.buttons.push_back({"Load New Filament", "LOAD_FILAMENT", "primary", false, -1});

        // Multi-word labels should be displayed as-is
        REQUIRE(data.buttons[0].label == "Load New Filament");
    }

    SECTION("Button gcode is stored separately from label") {
        PromptData data;
        data.title = "Test";
        data.buttons.push_back({"Preheat PLA", "M104 S200; M140 S60", "primary", false, -1});

        // The gcode can be different from the display label
        REQUIRE(data.buttons[0].label == "Preheat PLA");
        REQUIRE(data.buttons[0].gcode == "M104 S200; M140 S60");
    }
}
...
TEST_CASE("ActionPromptModal: Footer buttons appear separately", "[action_prompt][modal][footer]") {
    SECTION("Footer buttons are visually separated from regular buttons") {
        PromptData data;
        data.title = "Printer Action";
        data.buttons.push_back({"Continue", "CONTINUE", "primary", false, -1});
        data.buttons.push_back({"Skip", "SKIP", "secondary", false, -1});
        data.buttons.push_back({"Help", "SHOW_HELP", "info", true, -1}); // is_footer = true
        data.buttons.push_back({"Close", "CLOSE", "secondary", true, -1});

        // Footer buttons should be identifiable
        int regular_count = 0;
        int footer_count = 0;
        ...
        REQUIRE(regular_count == 2);
        REQUIRE(footer_count == 2);
    }

    SECTION("Footer-only prompt displays correctly") {
        PromptData data;
        data.title = "Information";
        data.text_lines.push_back("This is an informational message");
        data.buttons.push_back({"OK", "OK", "primary", true, -1});

        // Only footer buttons, no regular buttons
        REQUIRE(data.buttons.size() == 1);
        REQUIRE(data.buttons[0].is_footer == true);
    }

    SECTION("Mixed regular and footer buttons maintain order") {
        PromptData data;
        data.title = "Test";
        data.buttons.push_back({"A", "A", "", false, -1});
        data.buttons.push_back({"B", "B", "", true, -1}); // footer
        data.buttons.push_back({"C", "C", "", false, -1});
        data.buttons.push_back({"D", "D", "", true, -1}); // footer

        // Original order is preserved; UI will render them in sections
        REQUIRE(data.buttons[0].label == "A");
        REQUIRE(data.buttons[0].is_footer == false);
        REQUIRE(data.buttons[1].label == "B");
        REQUIRE(data.buttons[1].is_footer == true);
        REQUIRE(data.buttons[2].label == "C");
        REQUIRE(data.buttons[2].is_footer == false);
        REQUIRE(data.buttons[3].label == "D");
        REQUIRE(data.buttons[3].is_footer == true);
    }
}
...
TEST_CASE("ActionPromptModal: Button groups create horizontal layouts",
          "[action_prompt][modal][groups]") {
    SECTION("Buttons with same group_id should be in horizontal layout") {
        PromptData data;
        data.title = "Select Option";
        data.buttons.push_back({"Slot 1", "T0", "primary", false, 0});
        data.buttons.push_back({"Slot 2", "T1", "primary", false, 0});
        data.buttons.push_back({"Slot 3", "T2", "primary", false, 0});

        // All buttons have group_id = 0, so they should be in the same row
        for (const auto& btn : data.buttons) {
            REQUIRE(btn.group_id == 0);
        }
    }

    SECTION("Ungrouped buttons have group_id = -1") {
        PromptData data;
        data.title = "Test";
        data.buttons.push_back({"Standalone", "STANDALONE", "", false, -1});

        REQUIRE(data.buttons[0].group_id == -1);
    }

    SECTION("Multiple groups have different group_ids") {
        PromptData data;
        data.title = "Complex Layout";
        // Group 0: Temperature buttons
        data.buttons.push_back({"Hot", "HOT", "", false, 0});
        data.buttons.push_back({"Warm", "WARM", "", false, 0});
        data.buttons.push_back({"Cold", "COLD", "", false, 0});
        // Standalone
        data.buttons.push_back({"Separator", "SEP", "", false, -1});
        // Group 1: Speed buttons
        data.buttons.push_back({"Fast", "FAST", "", false, 1});
        data.buttons.push_back({"Slow", "SLOW", "", false, 1});

        // Verify group assignments
        REQUIRE(data.buttons[0].group_id == 0);
        REQUIRE(data.buttons[1].group_id == 0);
        REQUIRE(data.buttons[2].group_id == 0);
        REQUIRE(data.buttons[3].group_id == -1); // standalone
        REQUIRE(data.buttons[4].group_id == 1);
        REQUIRE(data.buttons[5].group_id == 1);
    }

    SECTION("Grouped and ungrouped buttons can coexist") {
        PromptData data;
        data.title = "Mixed";
        data.buttons.push_back({"Before", "BEFORE", "", false, -1});
        data.buttons.push_back({"Group A", "GA", "", false, 0});
        data.buttons.push_back({"Group B", "GB", "", false, 0});
        data.buttons.push_back({"After", "AFTER", "", false, -1});

        REQUIRE(data.buttons[0].group_id == -1);
        REQUIRE(data.buttons[1].group_id == 0);
        REQUIRE(data.buttons[2].group_id == 0);
        REQUIRE(data.buttons[3].group_id == -1);
    }
}
...
// ============================================================================
// Modal Display Tests (requires LVGL mocking)
// ============================================================================

TEST_CASE("ActionPromptModal: Component name and XML registration", "[action_prompt][modal][xml]") {
    SECTION("Modal has correct component name") {
        // When ActionPromptModal is implemented:
        // ActionPromptModal modal;
        // REQUIRE_THAT(modal.component_name(),
        //              Catch::Matchers::Equals("action_prompt_modal"));

        // The XML component should be registered [L014]:
        // In main.cpp: lv_xml_register_component_from_file(
        //     "action_prompt_modal", XML_COMPONENT("action_prompt_modal"));

        SUCCEED("Component name test placeholder");
    }

    SECTION("Modal callbacks are registered before XML creation [L013]") {
        // Per lesson L013: Callbacks must be registered before XML creation
        // The modal should call register_callbacks() in show() before
        // calling Modal::show(parent, attrs)

        // When implemented, verify callback registration order:
        // - Call lv_xml_register_event_cb() for button callbacks
        // - Then create XML via Modal::show()

        SUCCEED("Callback registration order test placeholder");
    }

    SECTION("Modal uses XML event_cb not lv_obj_add_event_cb [L007]") {
        // Per lesson L007: XML event callbacks only
        // Button click handlers should be defined in XML:
        // <event_cb trigger="clicked" callback="action_prompt_button_cb"/>
        // NOT using lv_obj_add_event_cb() in C++

        SUCCEED("XML event_cb test placeholder");
    }
}
