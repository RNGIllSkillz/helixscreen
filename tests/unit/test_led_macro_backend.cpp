...
TEST_CASE("MacroBackend: macro with custom actions", "[led][macro]") {
    helix::led::MacroBackend backend;

    helix::led::LedMacroInfo macro;
    macro.display_name = "LED Control";
    macro.on_macro = "LIGHTS_ON";
    macro.off_macro = "LIGHTS_OFF";
    macro.custom_actions = {{"Party Mode", "LED_PARTY"}, {"Night Light", "LED_NIGHTLIGHT"}};
    backend.add_macro(macro);

    REQUIRE(backend.macros().size() == 1);
    REQUIRE(backend.macros()[0].custom_actions.size() == 2);
    REQUIRE(backend.macros()[0].custom_actions[0].first == "Party Mode");
    REQUIRE(backend.macros()[0].custom_actions[0].second == "LED_PARTY");
}
